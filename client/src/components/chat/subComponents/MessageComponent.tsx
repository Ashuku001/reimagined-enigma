...
function MessageComponent({ message, unRead, list }: Props) {
    ...
    useEffect(() => {
        const checkIsVisible = (element: HTMLDivElement, id, messageId, status) => {
            const rect = element.getBoundingClientRect();
            if (rect.bottom <= window.innerHeight && id && messageId && status !== 'read') {
                updateMesStatus({  
                    variables: {
                        id: id,
                        messageId: messageId
        ...
        if (unRead?.current !== null) {
            checkIsVisible(unRead?.current, message.id, message.messageId, message.status)
        }
    ...
    }, [list, message.id, message.messageId, message.status, params.chatId, unRead, updateMesStatus,])


    return (
        <div 
            ref={(message?.status !== "read" && message?.from_customer === true) ? unRead : null} 
            ...
            <div className={`flex flex-col h-fit w-auto max-w-[180px] md:max-w-[350px] lg:max-w-[450px] p-[1px] rounded-lg  ${message?.from_customer ? ' mr-auto' : ' ml-auto '} ${message?.id < 0 ? 'bg-red-200' : ''}`}>
                <div className={` p-[1px] rounded-lg ${message?.from_customer ? ' bg-slate-300 dark:bg-slate-800 rounded-tl-none' : ' dark:bg-green-800 bg-green-300/80 rounded-tr-none'}`}>
                    {message?.context  &&
